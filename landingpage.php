...
.sidenav {
        ...
        .sidenav .closebtn {
            position: absolute;
            top: 0;
            left: 50%; /* Position the close button on the left side */
            font-size: 36px;
            margin-left: 50px;
        }
        ...
        .menu-icon {
            ...
            margin-right: 10px; /* Add space between the icon and button */
        }
...
<div class="container">
    ...
    <div class="header">
        ...
        <div id="mySidenav" class="sidenav">
                   ...
                   <!-- <a href="profile.php">Profile</a> -->       
                    <a href="loginacc.html">Logout</a>
                </div>
        <div class="user-actions">
            <button class="sign-out-button" onclick="redirectToCreate()">Sign Out</button>
            <img src="./assert/menu.png" alt="Menu Icon" class="menu-icon" onclick="openNav()">
            <script>
                    function openNav() {
    ...
    <div class="product-card" id="product-card">
        ...
        <form method="POST" id="favorite-form" style="display: inline-block; margin-top: 10px;">
            <input type="hidden" name="user_id" id="user-id" value="<?php echo $_SESSION['user_id']; ?>"> <!-- Assuming user_id is stored in the session -->
            <input type="hidden" name="product_id" id="favorite-product-id" value="">
            <button type="button" id="add-favorite-btn" class="add-favorite-button">Add to Favorites</button>
        </form>
    </div>
...
<script>
    ...
    function redirectToCreate() {
        window.location.href = './loginacc.html'; // Adjust this path to your sign-out script
    }

    // Redirect function for Profile Menu Icon
    function redirectToProfile() {
        window.location.href = './profile.html'; // Adjust this path to your profile page
    }
    ...
    const slideButton = document.querySelector('.slide-button');
                ...
                function endDrag() {
                    if (isDragging) {
                        ...
                        if (left >= maxSlide) {
                            ...
                            window.location.href = 'qr2.html'; // Replace with your target page
                        }
                ...
                document.getElementById('add-favorite-btn').addEventListener('click', function() {
                    ...
                    const productId = document.getElementById('favorite-product-id').value;

        // Check if the product ID is not empty
        if (!productId) {
            alert("Please select a product to add to favorites.");
            return;
        }

        // AJAX request
        const xhr = new XMLHttpRequest();
        xhr.open("POST", "add_favorite.php", true); // PHP script path to handle AJAX request
    ...
    // (Other JavaScript code from your original snippet goes here...)

    // Example: Default Product Setup & Search Input Event Listener
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            if (query.trim() === '') {
                setDefaultProduct(); // Show default product when search is empty
            ...
            fetch('products.json') // Adjust the path as necessary
                ...
                .then(data => {
                    ...
                    for (const key in data) {
                        if (data[key].name.toLowerCase().includes(query)) {
                            found = true;
                            // Update the product card with the matched product
                            productName.textContent = data[key].name;
                            productImage.src = data[key].img_path;
                            productSummary.textContent = data[key].summary;
                            productIngredients.innerHTML = ''; // Clear previous ingredients
                            data[key].ingredients.forEach(ingredient => {
                                const li = document.createElement('li');
                                li.textContent = ingredient;
                                productIngredients.appendChild(li);
                            });
                            productRating.innerHTML = `<span>Rating: ${data[key].overall_rating}</span>`;
                            favoriteProductId.value = key; // Set the product ID for favorites
                            break;
                        }
                    }

                    // If no product found, revert to the default product
                    if (!found) {
                        setDefaultProduct();
                    }
                })
